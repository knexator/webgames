...
export class Grid2D<T> {
    ...
    getV<S>(pos: Vec2, outOfBounds?: S): T | S {
        if (!inBounds(pos, this.size)) {
            ...
            throw new Error(`get at ${pos.toString()} was out of bounds, and no default argument was given`);
        }
    ...
    setV(pos: Vec2, value: T): void {
        if (!inBounds(pos, this.size)) {
            throw new Error(`can't set at ${pos.toString()}; out of bounds`);
        }
    ...
    find(discriminator: (pos: Vec2, element: T) => boolean): { pos: Vec2, element: T }[] {
        const result: { pos: Vec2, element: T }[] = [];
