...
export class NaiveSpriteGraphics {
    ...
    fullScreenTexture(texture: WebGLTexture) {
        let canvas = this.gl.canvas as HTMLCanvasElement;
        let size = new Vec2(canvas.clientWidth, canvas.clientHeight);
        this.draw("texture", {u_texture: texture}, size.scale(.5, new Vec2()), size, 0, Rectangle.unit);
    }
    ...
    textLineTopLeft(font: Font, text: string, top_left: Vec2, font_size: number, color: ColorArray) {
